...
def get_args():
    ...
    parser.add_argument('--prompt', type=str, default="You are seeing a photo of a cat. Describe this image in details <image>", help='Prompt')
...
def generate_responses_and_create_json(image_folder, prompt, model_id, output_file):
    ...
    for image_name in tqdm(os.listdir(image_folder)):
        ...
        if image_path.endswith(('.png', '.jpg', '.jpeg')):
            ...
            answer = answer.replace('The cat', '<sks>')
            answer = answer.replace('a cat', '<sks>')
