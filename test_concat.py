...
if __name__ == '__main__':
    ...
    prefix_tokens_2 = [f'<reserved{50+ latent_tokens_start_index+i}>' for i in range(config.prefix_token)]
    personalized_tokens_2 = [f'<reserved16250>'] + prefix_tokens_2
    sks_prompt_2 = f"{personalized_tokens_2[0]} is {''.join(personalized_tokens_2[1:])}."
    personalized_token_ids_2 = processor.tokenizer.convert_tokens_to_ids(personalized_tokens_2)
    ...
    try:
        if args.finetune:
            ...
            lm_head_path_2 = os.path.join(config.savedir, config.exp_name, 'mam', f'{config_test.iteration}-lmhead.pt')
            lm_head_2 = torch.load(lm_head_path_2, map_location='cuda')
        model.lm_head.weight.data[personalized_token_ids[:1]] = lm_head.to(model.lm_head.weight.data.device).to(model.dtype)[:1]
        model.lm_head.weight.data[personalized_token_ids_2[:1]] = lm_head_2.to(model.lm_head.weight.data.device).to(model.dtype)[:1]
        # model.lm_head.weight.data[personalized_token_ids[:1]] = lm_head.to(model.lm_head.weight.data.device).tso(model.dtype)[:1]
        ...
        if args.finetune:
            ...
            embedding_path_2 = f'{config.savedir}/{config.exp_name}/mam/{config_test.iteration}-token.pt'
        model.get_input_embeddings().weight.data[personalized_token_ids] = torch.load(embedding_path).to(model.device).to(model.dtype)
        model.get_input_embeddings().weight.data[personalized_token_ids_2] = torch.load(embedding_path_2).to(model.device).to(model.dtype)
    ...
    prompt = f"{sks_prompt} {sks_prompt_2} Can you describe <reserved16200>? Answer in detail."
    ...
    for i in tqdm(range(0, config_test.num_images, config_test.batch_size)):  # Step through by batch size
        ...
        gen_1 = "<reserved16201><reserved16202><reserved16203><reserved16204><reserved16205><reserved16206><reserved16207><reserved16208><reserved16209><reserved16210><reserved16211><reserved16212><reserved16213><reserved16214><reserved16215><reserved16216>"
        gen_2 = "<reserved16267><reserved16268><reserved16269><reserved16270><reserved16271><reserved16272><reserved16273><reserved16274><reserved16275><reserved16276><reserved16277><reserved16278><reserved16279><reserved16280><reserved16281><reserved16282>"
        # prompt_short = "A photo of <reserved16200> and <reserved16250>"
        prompt_short = 'A photo of <reserved16200>.'
        full_prompt = f"{sks_prompt} A photo of <reserved16200>.<reserved08706>{gen_1}"
        ...
        prompt_short = 'A photo of <reserved16200> with sunflowers.'
        full_prompt = f"{sks_prompt} A photo of <reserved16200> with sunflowers.<reserved08706>{gen_1}"
        inputs = processor([full_prompt] * config_test.batch_size, return_tensors="pt").to(model.device)
        generate_ids = model.generate(**inputs, multimodal_generation_mode="image-only", max_new_tokens=1026, do_sample=True)
        response_ids = generate_ids[:, inputs["input_ids"].shape[-1]:]
        pixel_values = model.decode_image_tokens(response_ids[:, 1:-1])
        ...
        prompt_short = 'A photo of <reserved16250>.'
        full_prompt = f"{sks_prompt_2} A photo of <reserved16250>.<reserved08706>{gen_2}"
        inputs = processor([full_prompt] * config_test.batch_size, return_tensors="pt").to(model.device)
        generate_ids = model.generate(**inputs, multimodal_generation_mode="image-only", max_new_tokens=1026, do_sample=True)
        response_ids = generate_ids[:, inputs["input_ids"].shape[-1]:]
        pixel_values = model.decode_image_tokens(response_ids[:, 1:-1])
        ...
        prompt_short = 'A photo of <reserved16250> with sunflowers'
        full_prompt = f"{sks_prompt_2} A photo of <reserved16250> with sunflowers.<reserved08706>{gen_2}"
        inputs = processor([full_prompt] * config_test.batch_size, return_tensors="pt").to(model.device)
        generate_ids = model.generate(**inputs, multimodal_generation_mode="image-only", max_new_tokens=1026, do_sample=True)
        response_ids = generate_ids[:, inputs["input_ids"].shape[-1]:]
        pixel_values = model.decode_image_tokens(response_ids[:, 1:-1])
        ...
        prompt_short = '<reserved16250> with <reserved16200>.'
        full_prompt = f"{sks_prompt} {sks_prompt_2} <reserved16250> with <reserved16200>.<reserved08706>{gen_2}{gen_1}"
        inputs = processor([full_prompt] * config_test.batch_size, return_tensors="pt").to(model.device)
        generate_ids = model.generate(**inputs, multimodal_generation_mode="image-only", max_new_tokens=1026, do_sample=True)
        response_ids = generate_ids[:, inputs["input_ids"].shape[-1]:]
        pixel_values = model.decode_image_tokens(response_ids[:, 1:-1])
